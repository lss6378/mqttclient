...
{
    MQTT_LOG_I("%s:%d %s()...\ntopic: %s, qos: %d, \nmessage:%s", __FILE_NAME__, __LINE__, __FUNCTION__,
...
static void mqtt_ack_handler_resend(mqtt_client_t* c, ack_handlers_t* ack_handler)
{
    ...
    MQTT_LOG_W("%s:%d %s()... resend %d package, packet_id is %d ", __FILE_NAME__, __LINE__, __FUNCTION__, ack_handler->type, ack_handler->packet_id);
}
    ...
    LIST_FOR_EACH_SAFE(curr, next, &c->mqtt_msg_handler_list) {
        ...
        if ((NULL != msg_handler->topic_filter) && (mqtt_is_topic_equals(msg_handler->topic_filter, handler->topic_filter))) {
            MQTT_LOG_W("%s:%d %s()...msg_handler->topic_filter: %s, handler->topic_filter: %s",
                        __FILE_NAME__, __LINE__, __FUNCTION__, msg_handler->topic_filter, handler->topic_filter);
            return 1;
        }
    }
...
static void mqtt_clean_session(mqtt_client_t* c)
{
    ...
    if (!(mqtt_list_is_empty(&c->mqtt_msg_handler_list))) {
        ...
        // MQTT_LOG_D("%s:%d %s() mqtt_msg_handler_list delete", __FILE_NAME__, __LINE__, __FUNCTION__);
...
static int mqtt_try_resubscribe(mqtt_client_t* c)
{
    ...
    MQTT_LOG_W("%s:%d %s()... mqtt try resubscribe ...", __FILE_NAME__, __LINE__, __FUNCTION__);

    if (mqtt_list_is_empty(&c->mqtt_msg_handler_list)) {
        // MQTT_LOG_D("%s:%d %s() mqtt_msg_handler_list is empty", __FILE_NAME__, __LINE__, __FUNCTION__);
        RETURN_ERROR(MQTT_SUCCESS_ERROR);
    }
    ...
    LIST_FOR_EACH_SAFE(curr, next, &c->mqtt_msg_handler_list) {
        ...
        if ((rc = mqtt_subscribe(c, msg_handler->topic_filter, msg_handler->qos, msg_handler->handler)) == MQTT_ACK_HANDLER_NUM_TOO_MUCH_ERROR)
            MQTT_LOG_W("%s:%d %s()... mqtt ack handler num too much ...", __FILE_NAME__, __LINE__, __FUNCTION__);
...
static int mqtt_try_do_reconnect(mqtt_client_t* c)
{
    ...
    MQTT_LOG_D("%s:%d %s()... mqtt try connect result is -0x%04x", __FILE_NAME__, __LINE__, __FUNCTION__, -rc);
...
static int mqtt_yield(mqtt_client_t* c, int timeout_ms)
{
    ...
    while (!platform_timer_is_expired(&timer)) {
        ...
        if (rc >= 0) {
            ...
            MQTT_LOG_E("%s:%d %s()... mqtt not connect", __FILE_NAME__, __LINE__, __FUNCTION__);
        } else {
            break;
        }
    }
...
static void mqtt_yield_thread(void *arg)
{
    ...
    state = mqtt_get_client_state(c);
        if (CLIENT_STATE_CONNECTED !=  state) {
            MQTT_LOG_W("%s:%d %s()..., mqtt is not connected to the server...",  __FILE_NAME__, __LINE__, __FUNCTION__);
    ...
    while (1) {
        ...
        if (MQTT_CLEAN_SESSION_ERROR == rc) {
            MQTT_LOG_W("%s:%d %s()..., mqtt clean session....", __FILE_NAME__, __LINE__, __FUNCTION__);
            network_disconnect(c->mqtt_network);
            mqtt_clean_session(c);
            goto exit;
        } else if (MQTT_RECONNECT_TIMEOUT_ERROR == rc) {
            MQTT_LOG_W("%s:%d %s()..., mqtt reconnect timeout....", __FILE_NAME__, __LINE__, __FUNCTION__);
        }
    }
...
static int mqtt_connect_with_results(mqtt_client_t* c)
{
    ...
    MQTT_LOG_I("%s:%d %s()... mqtt connect success...", __FILE_NAME__, __LINE__, __FUNCTION__);
    ...
    if (rc == MQTT_SUCCESS_ERROR) {
        if(NULL == c->mqtt_thread) {
            ...
            if (NULL != c->mqtt_thread) {
                ...
                network_release(c->mqtt_network);
                rc = MQTT_CONNECT_FAILED_ERROR;
                MQTT_LOG_W("%s:%d %s()... mqtt yield thread creat failed...", __FILE_NAME__, __LINE__, __FUNCTION__);
            }
        } else {
...
static uint32_t mqtt_read_buf_malloc(mqtt_client_t* c, uint32_t size)
{
    ...
    if (NULL == c->mqtt_read_buf) {
        MQTT_LOG_E("%s:%d %s()... malloc read buf failed...", __FILE_NAME__, __LINE__, __FUNCTION__);
        RETURN_ERROR(MQTT_MEM_NOT_ENOUGH_ERROR);
    }
...
static uint32_t mqtt_write_buf_malloc(mqtt_client_t* c, uint32_t size)
{
    ...
    if (NULL == c->mqtt_write_buf) {
        MQTT_LOG_E("%s:%d %s()... malloc write buf failed...", __FILE_NAME__, __LINE__, __FUNCTION__);
        RETURN_ERROR(MQTT_MEM_NOT_ENOUGH_ERROR);
    }
...
static int mqtt_init(mqtt_client_t* c)
{
    ...
    if (NULL == c->mqtt_network) {
        MQTT_LOG_E("%s:%d %s()... malloc memory failed...", __FILE_NAME__, __LINE__, __FUNCTION__);
        RETURN_ERROR(MQTT_MEM_NOT_ENOUGH_ERROR);
    }
...
int mqtt_keep_alive(mqtt_client_t* c)
{
    ...
    if (platform_timer_is_expired(&c->mqtt_last_sent) || platform_timer_is_expired(&c->mqtt_last_received)) {
        if (c->mqtt_ping_outstanding) {
            MQTT_LOG_W("%s:%d %s()... ping outstanding", __FILE_NAME__, __LINE__, __FUNCTION__);
...
int mqtt_release(mqtt_client_t* c)
{
    ...
    while ((CLIENT_STATE_INVALID != mqtt_get_client_state(c))) {
        ...
        if (platform_timer_is_expired(&timer)) {
            MQTT_LOG_E("%s:%d %s()... mqtt release failed...", __FILE_NAME__, __LINE__, __FUNCTION__);
            RETURN_ERROR(MQTT_FAILED_ERROR)
        }
    }
...
exit:
    ...
    if ((MQTT_ACK_HANDLER_NUM_TOO_MUCH_ERROR == rc) || (MQTT_MEM_NOT_ENOUGH_ERROR == rc)) {
        MQTT_LOG_W("%s:%d %s()... there is not enough memory space to record...", __FILE_NAME__, __LINE__, __FUNCTION__);
...
int mqtt_list_subscribe_topic(mqtt_client_t* c)
{
    ...
    if (mqtt_list_is_empty(&c->mqtt_msg_handler_list))
        MQTT_LOG_I("%s:%d %s()... there are no subscribed topics...", __FILE_NAME__, __LINE__, __FUNCTION__);

    LIST_FOR_EACH_SAFE(curr, next, &c->mqtt_msg_handler_list) {
        msg_handler = LIST_ENTRY(curr, message_handlers_t, list);
        /* determine whether a node already exists by mqtt topic, but wildcards are not supported */
        if (NULL != msg_handler->topic_filter) {
            MQTT_LOG_I("%s:%d %s()...[%d] subscribe topic: %s", __FILE_NAME__, __LINE__, __FUNCTION__, ++i ,msg_handler->topic_filter);
        }
    }
